...
class Helper():
    def __init__(self):
        self.summary_table = pd.DataFrame(columns = ['Model', 'Fine Method', 'Sub-Dataset', 'Train Size', 'tf Dataset', 'train loss', 'train acc', 'val loss', 'val acc', 'test loss', 'test acc'])

    def __PlotTrain(self, history, model_name="???"):
    ...
    def TrainModel(self, model, data_train, data_val, data_test, n_batch=32, num_epochs = 5):
        ...
        self.__PlotTrain(history, model.name)
    ...
    def TrainModelTFDS(self, model, data_train, data_val, data_test, n_batch=32, num_epochs=10):
    ...
    def EvaluateModel(self, model, X, y_true = None):
        try:
    ...
    def ModelPath(self, model_name, fine_method, sub_dataset, train_size, ds_bool):
        sep = '_'
        file_name = sep.join([model_name.replace('/', '_'), fine_method, sub_dataset, str(train_size), ds_bool])
        return file_name


    def SaveModel(self, model, model_name, fine_method, sub_dataset, train_size, ds_bool):
        file_name = ModelPath(model_name, fine_method, sub_dataset, train_size, ds_bool)
        model.save_weights('./Model/' + file_name)


    def LoadModel(self, model_name, fine_method, sub_dataset, train_size, ds_bool):
    ...
    def RecordToSummary(self, summary_table, model_name, fine_method, sub_dataset, train_size, ds_bool, score):
        """
        ...
        self.summary_table = pd.concat([summary_table, row], ignore_index = True)
